...
- The send-link modal and the "no access to this panel" notification name the target panel as a
  headline (`Client`) instead of its raw id in brackets (`[client]`). The exception messages still
  show the id as it is, since that is what a developer has to look for.

## 1.4.2 - 2026-09-19

### Changed

- The notification no longer repeats the link as plain text under the button. Laravel's mail
  template already prints it in the footer of any message with an action, so it showed up twice.
  The `mail.fallback` translation is gone with it.
